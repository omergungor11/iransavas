...
mkdir -p iransavas-tasks/{phases,active}
mkdir -p iransavas-config
mkdir -p iransavas-docs
mkdir -p iransavas-plans
```

`iransavas` = kullanicinin Adim 1'de verdigi prefix degeri.
...
Iceriklerde `iransavas` yerine kullanicinin verdigi GERCEK prefix degerini yaz.
...
2. `iransavas-docs/MEMORY.md` oku — kalici hafiza, teknik kararlar
3. `iransavas-tasks/task-index.md` oku — dashboard + tum task durumlari
4. `iransavas-tasks/active/session-notes.md` oku — onceki session notlari
...
3. `iransavas-tasks/task-index.md` oku — task durumu kontrol et
...
1. iransavas-tasks/task-index.md oku — aktif task var mi? Yapilan isleri listele
2. Task durumlarini guncelle (COMPLETED / IN_PROGRESS)
3. Session notu yaz → `iransavas-tasks/active/session-notes.md`:
...
### 4.5: Task Index (iransavas-tasks/task-index.md)
...
### 4.6: Phase 0 Detay (iransavas-tasks/phases/phase-0.md)
...
### Acceptance Criteria
- [ ] iransavas-tasks/ with task-index.md, phases/, active/
- [ ] iransavas-docs/ with MEMORY.md, CHANGELOG.md
- [ ] iransavas-config/ with workflow.md, conventions.md, tech-stack.md, agent-instructions.md
- [ ] iransavas-plans/ directory created
...
### 4.7: Session Notes (iransavas-tasks/active/session-notes.md)
...
**iransavas-config/tech-stack.md**: Secilen teknolojilerin versiyonlariyla doldur (guncel versiyonlari kullan). Runtime, backend framework, frontend framework, ORM, DB, styling, state management, testing vs.

**iransavas-config/agent-instructions.md**: Agent tipleri (backend, frontend, database, devops, docs) ve scope'lari yaz:
...
**iransavas-docs/MEMORY.md**: Proje adi, secilen tech stack, secilen ekstralar, project status (Phase 0 PENDING) ile doldurulmus baslat.

**iransavas-docs/CHANGELOG.md**:
```markdown
# Changelog
...
TAMAMEN OZELLESTIRILMIS ana konfigurasyon dosyasi yaz. Hicbir placeholder (iransavas, [PROJE_ADI] vs.) KALMAYACAK — hepsi gercek degerlerle degistirilmis olacak.
...
- HICBIR PLACEHOLDER birakma — `iransavas`, `[PROJE_ADI]` gibi seyler gercek degerlerle degistirilmis olmali
- Adim 3'teki template temizligini MUTLAKA yap (eski iransavas-tasks/, iransavas-docs/ vs. silinmeli)
